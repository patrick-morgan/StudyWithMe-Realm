exports = function(userIdToUpdate, friendId, newRelationshipType){
  ...
  */
  ...
  return userCollection.updateOne({ "_id": userIdToUpdate }, {$addToSet : {friends: {id: BSON.ObjectId().toString(), friendId: friendId, relationshipType: newRelationshipType}}})
  .then(result => {
    console.log(`Added new user relationship for ${userIdToUpdate}, updated friendId ${friendId} to ${newRelationshipType}`);
  }, error => {
    console.log(`Failed to add new relationship for user ${userIdToUpdate}, tried to add friendId ${friendId} to ${newRelationshipType}`);
    console.log(error);
  });
};